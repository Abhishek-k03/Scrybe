...
```
   ┌─────────────────────────┐
   │  React UI  (port 5173)  │     in-browser Babel · Tailwind
   │  dashboard · chat ·     │     chat · ingest · retrieval map
   │  ingest · retrieval     │
   └────────────┬────────────┘
                │  fetch /api/*
                ▼
   ┌─────────────────────────┐
   │  FastAPI  (port 8000)   │     async routes · CORS · pydantic
   └─┬────────┬────────┬─────┘
     │        │        │
     ▼        ▼        ▼
   ┌─────┐ ┌─────┐ ┌──────────┐
   │Play │ │Jina │ │ ChromaDB │     scraping · embeddings · vector store
   │wrigt│ │ v3  │ │ (local)  │
   └─────┘ └─────┘ └──────────┘
                │
                ▼
   ...
   ┌─────────────────────────┐
   │  Supabase (Postgres)    │     chats · messages (persistence)
   └─────────────────────────┘
```
...
```
   URL ─► Playwright ─► BeautifulSoup ─┐
                                       ├─► chunker ─► Jina v3 ─► ChromaDB
   File ─► pypdf / utf-8 ──────────────┘  800 tok    1024-d      cosine
                                          150 over   embed       index
                                                                    │
                                                                    ▼
                                                          { source_id, n_chunks }
```
...
```
   question ──► Jina v3 (query) ──► ChromaDB top-k ──┐
                                                     ▼
                                          chunks + metadata
                                                     │
                                                     ▼
                                        Groq · llama-3.3-70b
                                                     │
                                                     ▼
                                  answer + deduped citations
                                                     │
                              (if chat_id)           ▼
                              Supabase ◄── user / assistant rows
```
...
```
   embeddings (N × 1024) ─► SVD ─► top-2 components ─► 2-d points
                                          ▲                  │
                                          │                  ▼
                                  query embedding ──► query point + lines
```
